...
let strings = {}
let i18nCache = null
const pageContext = location.hostname === "genbs.github.io" ? "github" : "local"
document.querySelectorAll("[data-github-only]").forEach(el => el.toggleAttribute("hidden", pageContext !== "github"))

function applyStrings(root = document) {
	root.querySelectorAll("[data-i18n]").forEach(el => {
		const key = ["cli.hint", "proxy.desc"].includes(el.dataset.i18n)
			? `${el.dataset.i18n}.${pageContext}`
			: el.dataset.i18n
		el.innerHTML = strings[key] ?? el.innerHTML
	})
}
...
function serialCredentialsPacket(ssid, password) {
	const encoder = new TextEncoder()
	const ssidBytes = encoder.encode(ssid)
	const passwordBytes = encoder.encode(password)
	if (!ssidBytes.length || ssidBytes.length > 32 || ssidBytes.includes(0))
		throw new Error(t("flash.serial.invalid_ssid"))
	if (passwordBytes.length > 63 || passwordBytes.includes(0)) throw new Error(t("flash.serial.invalid_password"))

	const packet = new Uint8Array(7 + ssidBytes.length + passwordBytes.length)
	packet.set([0x52, 0x4c, 0x45, 0x44, 0x53, ssidBytes.length, passwordBytes.length])
	packet.set(ssidBytes, 7)
	packet.set(passwordBytes, 7 + ssidBytes.length)
	return packet
}

async function readSerialProvisioningResponse(port) {
	const reader = port.readable.getReader()
	const decoder = new TextDecoder()
	let response = ""
	const timeout = setTimeout(() => reader.cancel(), 10_000)
	try {
		while (true) {
			const { value, done } = await reader.read()
			if (done) break
			response += decoder.decode(value, { stream: true })
			if (response.includes("RLEDS:OK")) return true
			if (response.includes("RLEDS:ERROR")) return false
		}
	} finally {
...
const serialProvisionForm = document.getElementById("serial-provision-form")
const serialProvisionMessage = document.getElementById("serial-provision-message")
if (!("serial" in navigator)) serialProvisionForm.querySelector("button").disabled = true

serialProvisionForm.addEventListener("submit", async event => {
	event.preventDefault()
	const button = serialProvisionForm.querySelector("button")
	const passwordInput = document.getElementById("serial-password")
	let port
	button.disabled = true
	serialProvisionMessage.classList.remove("error")
	serialProvisionMessage.textContent = t("flash.serial.connecting")

	try {
		const packet = serialCredentialsPacket(document.getElementById("serial-ssid").value, passwordInput.value)
		...
		if (!(await readSerialProvisioningResponse(port))) throw new Error(t("flash.serial.failure"))
		passwordInput.value = ""
		serialProvisionMessage.textContent = t("flash.serial.success")
	} catch (err) {
		serialProvisionMessage.classList.add("error")
		serialProvisionMessage.textContent = err instanceof Error ? err.message : t("flash.serial.failure")
	} finally {
...
function deviceActions(key, idx, count) {
...
function createDeviceRow(key, config, idx, count, { onSelect } = {}) {
	...
	div.append(label, leds, deviceActions(key, idx, count))
	if (onSelect) div.addEventListener("click", () => onSelect(key))
	return div
}
...
async function renderDevices() {
	...
	devices.forEach(([key, config], idx) => {
		deviceList.appendChild(createDeviceRow(key, config, idx, devices.length))
	})
...
const devicesSectionList = document.getElementsByClassName("devices-section")
{
	...
	const defaultProxyUrl = "ws://localhost:8000"
	let url = localStorage.getItem("proxyUrl") || defaultProxyUrl
...
// mapping editor
{
	...
	function buildDeviceList() {
		...
		entries.forEach(([address, config], idx) => {
			const div = createDeviceRow(address, config, idx, n, {
				onSelect: address => {
					selected = address
					renderMapping()
				},
	...
	const hydraEl = document.getElementById("hydra-snippet")
	const hydraCopyBtn = document.getElementById("hydra-copy")
	const hydraOpenBtn = document.getElementById("hydra-open")
	const proxyInput = dialog.querySelector(".proxy-address")
...
function sendFrame() {
	...
	function hydraSnippet() {
		const clientUrl = "https://cdn.jsdelivr.net/npm/@reactive-leds/client@latest/build/reactive-leds.umd.js"
		const proxyUrl = proxyInput?.value || defaultProxyUrl
		return `// Hydra runs on hydra.ojack.xyz: start the proxy with
//   rleds proxy --origin https://hydra.ojack.xyz   (or --origin all to allow any page)
noise(1, 1)
  //.pixelate(${cols}, ${rows})
  .out()

await loadScript(${JSON.stringify(clientUrl)})

await rleds.begin(${JSON.stringify(proxyUrl)}, false)

const devices = await rleds.mapping(${exportSnippet()})

const pixels = new Uint8Array(width * height * 4)
update = () => {
  o0.regl.read({
    framebuffer: o0.getCurrent(),
    data: pixels,
  })
  ...
  devices.frame(pixels, width, height)
	...
	function hydraUrl() {
		let binary = ""
		for (const byte of new TextEncoder().encode(hydraSnippet())) binary += String.fromCharCode(byte)
	...
	function persistMapping() {
		...
		if (hydraEl) hydraEl.textContent = hydraSnippet()
		if (hydraOpenBtn) hydraOpenBtn.disabled = window.devices.size === 0
	}
	...
	function renderMappingPreview() {
		...
		for (const [address, config] of window.devices.entries()) {
			...
			const leds = []
			for (let i = 0; i < config.num_leds; i++) leds.push([out[i * 4], out[i * 4 + 1], out[i * 4 + 2], out[i * 4 + 3]])
	...
	if (hydraCopyBtn) wireCopy(hydraCopyBtn, hydraSnippet)
	if (hydraOpenBtn) hydraOpenBtn.addEventListener("click", () => window.open(hydraUrl(), "_blank", "noopener"))
