...
type BonjourServiceEvents = {
	deviceUp: (device: TBonjourClient) => void
	deviceDown: (device: TBonjourClient) => void
}

export default class BonjourService extends EventEmitter<BonjourServiceEvents> {
	private devices = new Map<string, TBonjourClient>()
	...
	findByAddress(address: string): TBonjourClient | void {
		return this.devices.get(address)
	}

	findByHostname(hostname: string): TBonjourClient | void {
		return [...this.devices.values()].find(device => device.hostname === hostname)
	}

	private serviceToClient(service: RemoteService): TBonjourClient | void {
