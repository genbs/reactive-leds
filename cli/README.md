...
- `bt-scan [timeout]` - scan devices over Bluetooth. The timeout is in milliseconds (default: `5000`).
...
- `serial-scan` - list available serial ports.
- `credential [port] [ssid]` - send WiFi credentials over USB. If `port` is omitted, uses the only available port or asks which one to select; missing SSID and password are prompted.
- `proxy [host] [port] [device_port] [options]` - start the WebSocket proxy between browser clients and the firmware. Defaults: `host=127.0.0.1`, `port=8000`, `device_port=4210`. Add `--ui` to serve the browser UI locally, or `--origin <origin>` to authorize a specific remote page. Scans the LAN every 10 seconds and shows discovered devices in real time.
...
It uses the binary multiplexed protocol consumed by `@reactive-leds/client`. WS responses have the form `[requestId, ...payload]` (no `PacketType` byte): the browser matches each response to its request by `requestId`. The device-side wire format is documented in [`shared/README.md`](https://github.com/genbs/reactive-leds/blob/master/shared/README.md).

To run the bundled web interface locally, use `rleds proxy --ui` and open `http://localhost:8000`. To connect from the GitHub Page, authorize its origin explicitly:

```bash
rleds proxy --origin https://genbs.github.io
```

The page connects to `ws://localhost:8000`. Browsers may report active mixed content because the page is HTTPS while the local WebSocket is not; use `--ui` for the cleanest local experience. Remote origins remain denied unless explicitly listed; comma-separate multiple origins when needed.

To let any remote page connect (e.g. a public tool or an editor like [Hydra](https://hydra.ojack.xyz/)), allow all origins:

```bash
rleds proxy --origin all
```

The keyword `all` is equivalent to the `*` wildcard but is safe to type in interactive shells (where `*` would be expanded by the shell); `*` is still accepted when quoted.

> ⚠️ `--origin all` lets **any** website you visit drive the proxy while it runs. Prefer the explicit list when you can name the origins.
...
Ordinary `SET_LEDS` commands use packet id `0`, so they do not update or reset `arrival-gaps`, `seq-lost` or `seq-reordered`. The benchmark sends packet id `1` as a start marker: it resets the current run's maximum-gap measurement and sequence baseline, but does not clear the cumulative histogram or sequence counters. Its measured frames use ids `2`–`255`, wrapping back to `2`.
...
After flashing, the most direct method is the **Configure WiFi via USB** form on the install page, available in browsers with Web Serial support (Chrome and Edge).

From a terminal, connect the device over USB and use:

`serial-scan` → `credential` → device reboots → WiFi ready.

Bluetooth remains useful when the device is already installed or only its power connection is accessible:
