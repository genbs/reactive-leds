...
function createReport(
	...
	const hist = after.arrivalGapHist.map((count, i) => count - before.arrivalGapHist[i])
	const maxGapAgeMs = after.arrivalGapMaxAgeS * 1000
...
function printReport(report: BenchmarkReport) {
	...
	console.log(`  udp-read/attempted ${fmtPct(report.metrics.udpReadPct)}  set-leds ${fmtPct(report.metrics.deliveryPct)}  shown-rate ${report.metrics.shownFps.toFixed(1)}fps  rmt-drop-rate ${fmtPct(report.metrics.rmtDropPct, 3)}  loop-max-gap ${after.protocolLoopMaxGapMs}ms since boot  rssi ${rssi}`)
