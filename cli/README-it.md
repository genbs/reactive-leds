...
Language: [English](https://github.com/genbs/reactive-leds/blob/master/cli/README.md) | [Italiano](https://github.com/genbs/reactive-leds/blob/master/cli/README-it.md)

Questo pacchetto è la CLI per interagire con i dispositivi `reactive-leds` in rete.
Con il comando `rleds` puoi interagire con i device, effettuare il provisioning via USB o BLE e avviare il proxy WebSocket, tutto dal terminale.
...
- `scan [port] [timeout]` - cerca device nella LAN tramite broadcast UDP. Valori predefiniti: `port=4210`, `timeout=1500` ms. Ogni risultato include anche l'hostname del device (dalla sua configurazione), così puoi associare l'IP all'etichetta applicata al case. I risultati vengono salvati nella cache su disco (`/tmp/reactive-leds-scan.json` su macOS/Linux) per velocizzare i comandi successivi; la cache scade dopo 5 minuti.
...
- `leds <target> <leds_package>` - aggiorna LED contigui. Il pacchetto è `<start_index>` seguito da uno o più gruppi `<r>,<g>,<b>,<w>`: `2,255,0,0,0` rende rosso il terzo LED senza modificare gli altri. Ogni valore è tra 0 e 255.
- `bt-scan [timeout]` - cerca device via Bluetooth. Il timeout è espresso in millisecondi (valore predefinito: `5000`).
...
- `serial-scan` - elenca le porte seriali disponibili.
- `credential [port] [ssid]` - invia le credenziali Wi-Fi tramite USB. Se `port` è omessa, usa l'unica porta disponibile oppure chiede quale scegliere; SSID e password mancanti vengono richiesti al prompt.
- `proxy [host] [port] [device_port] [opzioni]` - avvia il proxy WebSocket tra client browser e firmware. Valori predefiniti: `host=127.0.0.1`, `port=8000`, `device_port=4210`. Aggiungi `--ui` per servire localmente l'interfaccia browser, oppure `--origin <origine>` per autorizzare una pagina remota specifica.
...
- `status [target]` - legge lo stato del device (uptime, heap, RSSI Wi-Fi e contatori dei frame a runtime). Usa `all` o ometti `target` per interrogare tutti i device scoperti.
...
Usa il protocollo binario multiplexato consumato da `@reactive-leds/client`. Le risposte WS hanno formato `[requestId, ...payload]` (senza byte `PacketType`): il browser le abbina alla richiesta tramite il `requestId`. Wire format del protocollo device documentato in [`shared/README-it.md`](https://github.com/genbs/reactive-leds/blob/master/shared/README-it.md).

Per avviare l'interfaccia inclusa nel pacchetto, usa `rleds proxy --ui` e apri `http://localhost:8000`. Per collegarti dalla GitHub Page, autorizza esplicitamente la sua origine:

```bash
rleds proxy --origin https://genbs.github.io
```

La pagina usa `ws://localhost:8000`. Il browser può segnalare contenuto misto attivo perché la pagina è HTTPS mentre il WebSocket locale non lo è; `--ui` offre l'esperienza locale più pulita. Le origini remote restano bloccate se non sono elencate esplicitamente; per più origini usa una lista separata da virgole.

Per permettere a qualsiasi pagina remota di collegarsi (es. un tool pubblico o un editor come [Hydra](https://hydra.ojack.xyz/)), consenti tutte le origini:

```bash
rleds proxy --origin all
```

La keyword `all` equivale al wildcard `*` ma si può digitare senza virgolette nelle shell interattive (dove `*` verrebbe espanso dalla shell); `*` resta comunque accettato se quotato.

> ⚠️ `--origin all` permette a **qualsiasi** sito che visiti di usare il proxy finché è attivo. Preferisci la lista esplicita quando conosci le origini.
...
```
$ rleds proxy
Proxy: ws://127.0.0.1:8000  devices: 1
...
I normali comandi `SET_LEDS` usano packet id `0`, quindi non aggiornano né azzerano `arrival-gaps`, `seq-lost` o `seq-reordered`. Il benchmark invia il packet id `1` come marker iniziale: azzera la misura del massimo gap della run corrente e la baseline di sequenza, ma non cancella l’istogramma o i contatori di sequenza cumulativi. I frame misurati usano gli ID `2`–`255`, poi ripartono da `2`.
...
Dopo il flash, il metodo più diretto è il form **Configura WiFi via USB** nella pagina di installazione, disponibile nei browser con Web Serial (Chrome ed Edge).

Da terminale, collega il dispositivo via USB e usa:

`serial-scan` → `credential` → riavvio dispositivo → Wi-Fi pronto.

Il Bluetooth rimane un'alternativa utile quando il dispositivo è già montato o raggiungibile solo tramite alimentazione:
